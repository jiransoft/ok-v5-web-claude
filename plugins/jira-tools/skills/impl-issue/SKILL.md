...
구현 결과 댓글 초안을 다음 템플릿으로 작성한다. 구현으로 달라진 동작을 먼저 설명하고, 검증 결과와 남은 검증, 작업 상태를 구분한다. `남은 검증`은 해당할 때만 포함하고, 기존 데이터 영향이 없으면 `적용 범위`로 제목을 줄인다.

```markdown
## 구현 결과

{어떤 조건에서 발생하던 문제가 어떻게 달라졌는지. 기능 추가라면 새로 제공하는 동작}

## 수정 내용

1. {주요 변경과 변경 이유}

2. {변경 적용 범위와 보존한 기존 동작}

## 변경 파일

- `{파일 경로}`: {변경 역할. 신규 파일이면 명시}

## 검증 결과

- 실행 환경: {로컬·CI·QC 등 실제 검증 환경}
- 단위 테스트: {실행 대상, 통과·실패·건너뜀 건수와 주요 검증 내용}
- 통합 테스트: {실행 대상과 실제 확인한 처리·저장·응답 범위}
- 기타 검사: {실행한 검사와 결과}

## 남은 검증

- {미검증 환경 또는 시나리오}: {확인 절차} → {기대 결과}

## 적용 범위 및 기존 데이터 영향

{배포 후 변경되는 동작과 적용 대상}
{기존 데이터의 자동 갱신 여부, 별도 보정 필요 여부와 이번 작업 포함 여부}

## 작업 상태

- 브랜치: `{브랜치명}`
- 커밋: {이번 변경의 커밋 해시. 사용자 지시로 커밋을 생략했다면 미커밋으로 명시}
- 푸시·배포: {각각의 실제 수행 상태}
- PR: {있으면 링크. 없으면 생략}
```
...
- 구현 결과는 사용자 관점의 변경 전후 동작으로 요약한다. 수정 내용에는 실제 반영한 변경만 적고, 추가 제안이나 미완료 작업을 구현한 것처럼 표현하지 않는다
- 파일 경로와 심볼은 인라인 코드로 표시한다. 변경 파일에는 구현·테스트 등 각 파일의 역할을 짧게 적고, 코드 블록이나 표는 설명에 필요한 경우에만 추가한다. 번호 목록의 개수는 실제 변경에 맞춰 조정한다
- 검증 결과에는 실제 실행한 대상과 결과를 적는다. 테스트 건수는 실행 로그나 리포트로 확인된 경우에만 기재하고, 실패·건너뜀을 통과 건수에 포함하지 않는다. 테스트별 환경이 다르면 각 항목에 구분해서 적는다
- 통합 테스트는 실제 의존 서비스 사용 여부와 확인한 범위를 명시한다. 로컬에서 정책 게시·DB 저장을 검증한 결과를 QC·운영 환경 또는 화면까지 검증한 것처럼 표현하지 않는다
- 관련 테스트나 검사를 실행하지 않았다면 미수행과 사유를 적는다. 남은 검증에는 필요한 환경과 확인 절차, 기대 결과를 적어 후속 QA가 실행할 수 있도록 한다
- 적용 범위에는 영향을 받는 기능과 유지한 기존 동작을 설명한다. 기존 데이터가 관련되면 생성·저장·조회 구조를 근거로 자동 갱신 여부와 보정 포함 여부를 명시한다. 과거 로그 보정에는 당시 보존된 데이터를 사용하며 현재 값으로 과거 값을 대신하지 않는다
- 작업 상태는 댓글 작성 시점에 확인한 사실만 적는다. 9단계에서 생성한 이번 변경의 커밋을 기록하고, 사용자 지시로 미커밋 상태라면 기존 HEAD를 이번 변경의 커밋으로 표시하지 않는다. 푸시·배포는 각각 수행 여부를 구분하고 확인하지 못한 상태는 미확인으로 적는다. 상태 기재를 위해 푸시·배포·PR 생성을 추가 수행하지 않는다
