---
name: resolve-issue
description: Jira 이슈를 분석하여 확인 근거, 수정 제안, 검증 계획과 분석 범위를 정리한 댓글을 등록합니다
...
Jira 이슈를 조회하고, 코드베이스를 분석하여 확인 근거, 수정 제안, 검증 계획과 분석 범위를 정리한 댓글을 등록한다.
...
다음 템플릿에 따라 댓글 내용을 구성한다. 각 항목은 짧은 설명 문단으로 작성하고, 수정 제안과 검증 계획은 번호 목록으로 정리한다. `기존 데이터 영향`은 관련 있을 때만 포함한다.

```markdown
## 분석 요약

{어떤 조건에서 무엇이 기대와 다르게 동작하는지}
{코드에서 확인한 원인. 확정하지 못했다면 원인 후보와 추가 확인 사항}

## 확인 근거

{관련 파일·심볼과 문제를 일으키는 조건}
{입력 → 저장 → 조회·표시 중 문제가 발생하는 단계와 근거}
{실제 환경 데이터 등 아직 확인하지 못한 사항}

## 수정 제안

1. {수정할 파일·심볼과 구체적인 변경 내용}

2. {변경을 적용할 범위와 유지해야 할 기존 동작}

3. {재사용할 구현 및 필요한 후속 변경}

## 검증 계획

1. {문제 발생 조건} → {수정 후 기대 결과}

2. {영향받지 않아야 하는 조건} → {유지되어야 할 결과}

3. {실제 환경 확인 절차} → {확인할 저장 값·API 응답·화면 표시}

## 기존 데이터 영향

{수정 후 새로 생성되는 데이터와 기존 데이터에 미치는 영향}
{별도 보정 필요 여부와 보정에 필요한 당시 데이터}

## 분석 범위 및 수행 내역

- 분석 근거: {Jira 본문·댓글·외부 자료, 저장소·브랜치·커밋 등}
- 코드 수정: {미수행 / 수행 내용}
- 실행 재현·테스트: {미수행 / 수행 내용과 결과}
- 추가 확인: {남은 확인 사항. 없으면 생략}
```

**작성 규칙:**
- 분석 요약 → 확인 근거 → 수정 제안 → 검증 계획 순으로 내용을 연결한다. 확인 근거에서는 문제 지점과 데이터 흐름을 함께 설명하여 중복을 줄인다. 이슈와 관련 없는 단계나 항목을 억지로 채우지 않고 번호 목록의 개수도 필요한 만큼 조정한다
- 파일 경로와 심볼은 인라인 코드로 표시하고, 라인 번호는 직접 확인한 경우에만 기재한다. 코드 블록·표·수정 코드 예시는 설명에 필요한 경우에만 추가한다
- 코드에서 확인된 결함과 실제 환경에서 확인된 현상을 구분한다. 정적 분석만으로 데이터 누락·저장 실패·실행 재현까지 확인했다고 단정하지 않는다. 불확실한 부분은 "추가 확인 필요"로 표기한다
- 수정 제안은 바로 구현할 수 있을 정도로 구체적으로 작성하되, 제안한 변경을 이미 적용한 것처럼 표현하지 않는다. 재사용 가능한 구현과 변경 적용 범위, 유지해야 할 기존 동작을 함께 설명한다
- 검증 계획은 수행할 조건·절차와 기대 결과를 짝지어 적는다. 회귀 테스트와 기존 동작 유지 여부를 포함하고, 다국어 등 관련 변형도 고려한다. 이미 실행한 테스트 결과는 `분석 범위 및 수행 내역`에 적고, 미실행 항목을 검증 완료로 표현하지 않는다
- 기존 로그가 관련되면 생성·저장·조회 구조를 근거로 자동 복구 여부를 판단한다. 과거 로그 보정은 당시 보존된 스냅샷 등 이력 데이터를 기준으로 제안하며, 현재 값을 과거 값처럼 사용하여 재구성하지 않는다
- 분석 범위 및 수행 내역에는 실제 사용한 자료와 코드 기준, 코드 수정 및 실행 재현/테스트 수행 여부를 명시한다. 이 항목은 수행 사실을 기록하는 용도이며 분석 요청의 작업 범위를 확장하지 않는다
